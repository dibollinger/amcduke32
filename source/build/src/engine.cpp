...
# endif
    ...
    for (int i = 0; i < numwalls; ++i)
    {
        ...
        if (tile.newtile < MAXTILES && !waloff[tile.newtile])
...
static int32_t amc_strip_mapversion_flags(int32_t version)
{
    ...
    return version & ~(MAPVERSION_FLAG_AMC5_0 | MAPVERSION_FLAG_AMC4_5 | MAPVERSION_FLAG_AMC4_0);
}
...
//        return saveboard_maptext(filename, dapos, daang, dacursectnum);
    ...
    if (maxpicnum >= AMC4_0_MAXTILES) mapversion |= MAPVERSION_FLAG_AMC4_5;
    if (maxpicnum >= AMC4_5_MAXTILES) mapversion |= MAPVERSION_FLAG_AMC5_0;
